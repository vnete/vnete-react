...
describe("editor/deserialize", function () {
    describe("text messages", function () {
        ...
        it("@room pill", function () {
            const parts = normalize(
                parseEvent(
                    textMessage("text message for @room"),
                    createPartCreator()
                )
            );
        ...
        it("emote", function () {
            ...
            const parts = normalize(
                parseEvent(textMessage(text, "m.emote"), createPartCreator())
            );
            expect(parts.length).toBe(1);
            expect(parts[0]).toStrictEqual({
                type: "plain",
                text: "/me says DON'T SHOUT!",
            });
        });
    });
    describe("html messages", function () {
        it("inline styling", function () {
            ...
            const parts = normalize(
                parseEvent(htmlMessage(html), createPartCreator())
            );
            expect(parts.length).toBe(1);
            expect(parts[0]).toStrictEqual({
                type: "plain",
        ...
        it("quote", function () {
            const html =
                "<blockquote><p><em>wise</em><br><strong>words</strong></p></blockquote><p>indeed</p>";
            const parts = normalize(
                parseEvent(htmlMessage(html), createPartCreator())
            );
            expect(parts.length).toBe(6);
        ...
        it("user pill", function () {
            const html =
                'Hi <a href="https://link.vnete.net/#/@alice:hs.tld">Alice</a>!';
            const parts = normalize(
                parseEvent(htmlMessage(html), createPartCreator())
            );
            expect(parts.length).toBe(3);
        ...
        it("room pill", function () {
            const html =
                'Try <a href="https://link.vnete.net/#/#room:hs.tld">#room:hs.tld</a>?';
            const parts = normalize(
                parseEvent(htmlMessage(html), createPartCreator())
            );
            expect(parts.length).toBe(3);
        ...
        // failing likely because of https://github.com/vnete/vnete-chat/issues/10316
        ...
        it("ordered lists", function () {
            const html =
                "<ol><li>Start</li><li>Continue</li><li>Finish</li></ol>";
            const parts = normalize(
                parseEvent(htmlMessage(html), createPartCreator())
            );
            expect(parts.length).toBe(5);
        ...
        it("emote", function () {
            ...
            const parts = normalize(
                parseEvent(htmlMessage(html, "m.emote"), createPartCreator())
            );
            expect(parts.length).toBe(1);
            expect(parts[0]).toStrictEqual({
                type: "plain",
                text: "/me says _DON'T SHOUT_!",
            });
        });
    });
});
